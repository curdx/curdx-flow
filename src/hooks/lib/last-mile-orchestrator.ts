...
// frontend-design, and chrome-devtools-mcp without asking the user to pick a
// skill manually.
...
const CODING_PROMPT_RE =
  /\b(implement|fix|debug|build|add|update|refactor|release|publish|test|verify|ui|ux|frontend|backend|plugin|hook|skill|mcp|agent|doctor|npm|tag)\b|实现|修复|开发|构建|新增|更新|重构|发布|测试|验证|前端|后端|页面|插件|钩子|技能|报错|失败|定位|排查/i;
const RELEASE_RE =
  /\b(release|publish|deploy|ship|tag|npm|version|changelog)\b|发布|部署|上线|打包|版本|标签/i;
const FAILURE_RE =
  /\b(fail|failed|failure|error|stuck|retry|debug|broken|regression)\b|失败|报错|卡住|重试|排查|定位|回归/i;
