...
#
# Functions provided:
#   curdx_get_specs_dirs()   - Returns newline-separated list of configured dirs
#   curdx_find_spec(name)    - Returns full path to spec, handles disambiguation
#   curdx_list_specs()       - Returns all specs as "name|path" pairs
#   curdx_resolve_current()  - Returns full path from .current-spec
#   curdx_get_default_dir()  - Returns first specs_dir (for new spec creation)

CURDX_CWD="${CURDX_CWD:-$(pwd)}"
CURDX_SETTINGS_FILE="$CURDX_CWD/.claude/curdx-flow.local.md"
CURDX_DEFAULT_SPECS_DIR="./specs"
...
# Internal: Validate CURDX_CWD exists
_curdx_validate_cwd() {
    if [ ! -d "$CURDX_CWD" ]; then
        _curdx_warn "CURDX_CWD does not exist: $CURDX_CWD"
        return 1
    fi
    return 0
}
...
curdx_get_specs_dirs() {
    # Validate CURDX_CWD first
    if ! _curdx_validate_cwd; then
        echo "$CURDX_DEFAULT_SPECS_DIR"
        return 0
    fi
    ...
    while IFS= read -r dir; do
        ...
        dir=$(_curdx_normalize_path "$dir")

        # Validate path format
        if [[ "$dir" == /* ]] && [[ "$dir" != "$CURDX_CWD"* ]]; then
            # Absolute path outside CURDX_CWD - check if it exists
            if [ ! -d "$dir" ]; then
                _curdx_warn "Skipping invalid absolute path in specs_dirs: $dir (does not exist)"
                continue
            fi
        elif [[ "$dir" == ./* ]] || [[ "$dir" != /* ]]; then
            # Relative path - check if it exists
            if [ ! -d "$CURDX_CWD/$dir" ]; then
                _curdx_warn "Skipping invalid path in specs_dirs: $dir (directory not found at $CURDX_CWD/$dir)"
                continue
            fi
        fi
    ...
    if [ -z "$validated_dirs" ]; then
        _curdx_warn "No valid paths in specs_dirs, using default: $CURDX_DEFAULT_SPECS_DIR"
        echo "$CURDX_DEFAULT_SPECS_DIR"
    else
        echo "$validated_dirs"
    fi
...
curdx_get_default_dir() {
    local first_dir
    first_dir=$(curdx_get_specs_dirs | head -1)
    # Normalize path before returning
    _curdx_normalize_path "$first_dir"
}
...
curdx_resolve_current() {
    ...
    local current_spec_file="$CURDX_CWD/$default_dir/.current-spec"
    ...
    if [ -f "$current_spec_file" ]; then
        ...
        if [ -z "$content" ]; then
            _curdx_warn ".current-spec file is empty"
            return 1
        fi

        # Normalize the path
        content=$(_curdx_normalize_path "$content")
...
curdx_find_spec() {
    ...
    name=$(_curdx_normalize_path "$name")
    ...
    if [ $count -eq 0 ]; then
        ...
        echo "$found" | nl >&2
        echo "Specify full path: /curdx-flow:switch <path>" >&2
        return 2
    fi
}
...
curdx_list_specs() {
    ...
    while IFS= read -r dir; do
        ...
        if [ -d "$CURDX_CWD/$dir" ]; then
            ...
            done < <(find "$CURDX_CWD/$dir" -maxdepth 1 -mindepth 1 -type d 2>/dev/null)
        fi
    done <<< "$dirs"
}
