...
2. TeamCreate(team_name: "review-tasks-$spec")
3. TaskCreate(subject: "Spec-compliance review of tasks.md",
              activeForm: "Reviewing tasks (spec-compliance)")
   TaskCreate(subject: "Code-quality review of tasks.md",
...
6. # Persist verdicts under verificationBlocks.tasks.reviews via merge-state (FR-T3 — never hand-edit state):
...
**Anti-rationalization**: the coordinator MUST NOT pass spec-reviewer's findings into the code-quality-reviewer prompt (or vice versa). Both verdicts are stored verbatim under `verificationBlocks.tasks.reviews`. See [`references/two-stage-review.md`](../references/two-stage-review.md) §2.
