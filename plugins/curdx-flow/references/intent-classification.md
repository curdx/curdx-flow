...
- `/curdx-flow:start` -> Auto-detect: resume active or ask for new
- `/curdx-flow:start user-auth` -> Resume or create user-auth
- `/curdx-flow:start user-auth Add OAuth2` -> Create user-auth with goal
- `/curdx-flow:start user-auth --fresh` -> Force new, overwrite if exists
- `/curdx-flow:start "Build auth with JWT" --quick` -> Quick mode with goal string
- `/curdx-flow:start my-feature "Add logging" --quick` -> Quick mode with name+goal
...
- `/curdx-flow:start my-feature "Add logging" --tasks-size coarse` -> Coarse granularity (10-20 tasks)
...
3. If not found: error "Spec '$name' not found. Provide goal: /curdx-flow:start $name 'your goal' --quick"
