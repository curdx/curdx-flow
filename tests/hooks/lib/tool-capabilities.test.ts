...
describe("tool capability router", () => {
  ...
  it("routes frontend runtime work to UI design and browser verification tools", () => {
    const recs = recommendToolCapabilities({
      ...
      availableCapabilities: ["frontend-design", "ui-ux-pro-max", "chrome-devtools-mcp", "context7"],
    });
  ...
  it("does not recommend tools while missing code roots block the route", () => {
    const recs = recommendToolCapabilities({
      ...
      risk: "medium",
      missingRoots: 1,
      availableCapabilities: ["frontend-design", "ui-ux-pro-max", "chrome-devtools-mcp"],
    });
  ...
  it("does not treat companion capability names as frontend/browser stack intent", () => {
    const recs = recommendToolCapabilities({
      goal: "Use claude-mem pua context7 sequential-thinking chrome-devtools-mcp frontend-design front-end-design ui-ux-pro-max uiuxmax ui ux pro max for better routing",
      route: "lite-spec",
      risk: "medium",
    });
