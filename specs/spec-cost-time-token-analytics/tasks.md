...
### Task 0.1: Create sample-with-usage.jsonl fixture
...
### Task 1.1: Add UsageRow + AggregateBucket + Severity + Recommendation types
...
### Task 1.2: [P] Create pricing.ts with PRICING + alias + LAST_UPDATED
...
### Task 1.3: [P] Create cost.ts skeleton with computeCost basic formula
...
### Task 1.4: [VERIFY] Quality checkpoint: typecheck + build
...
### Task 1.5: Wire --cost-summary CLI flag in index.ts
...
### Task 1.6: [VERIFY] Quality checkpoint: typecheck + build
...
### Task 1.7: [POC MILESTONE] Manual smoke — totalCost.usd non-null
...
### Task 1.8: [VERIFY] POC milestone gate
...
### Task 2.1: Extend schema-map JSON with assistant.message.usage fields
...
### Task 2.2: [P] Add extractTrailerUsage with regex parsing in cost.ts
...
### Task 2.3: [P] Wire trailer extraction into extractUsageRowsFromEvents
...
### Task 2.4: [VERIFY] Quality checkpoint: typecheck + build + test:analyze
...
### Task 2.5: Add aggregateBy three-level join in cost.ts
...
### Task 2.6: Build specPhaseMap from state files in index.ts
...
### Task 2.7: Wire --by-spec / --by-phase / --by-task / --top flags in index.ts
...
### Task 2.8: [VERIFY] Quality checkpoint: typecheck + build
...
### Task 2.9: Wire cost branch JSON output costBreakdown structure
...
### Task 2.10: Add lastCostSummary cache discriminator in StateFile
...
### Task 2.11: [VERIFY] Quality checkpoint: typecheck + build + test:analyze
...
### Task 2.12: Smoke verify costBreakdown + 5 flags
...
### Task 3.1: [P] Create pricing.test.ts — 3 model × 5 field + alias
...
### Task 3.2: [P] Create cost.test.ts — computeCost ±0.001 USD precision
...
### Task 3.3: [VERIFY] Quality checkpoint: typecheck + test:analyze
...
### Task 3.4: [P] cost.test.ts — extractTrailerUsage regex test
...
### Task 3.5: [P] cost.test.ts — aggregateBy 3-level snapshot
...
### Task 3.6: [VERIFY] Quality checkpoint: typecheck + test:analyze
...
### Task 3.7: [P] Create recommend.ts skeleton (8 rules, no impl yet)
...
### Task 3.8: [P] Create recommend.test.ts — MAD 11 edge cases
...
### Task 3.9: [VERIFY] Quality checkpoint: typecheck + test:analyze
...
### Task 3.10: Extend integration.test.ts with sample-with-usage.jsonl
...
### Task 3.11: Add R3 trailer attribution assertion in integration
...
### Task 3.12: Add requestId dedup regression assert in integration
...
### Task 3.13: [VERIFY] Quality checkpoint: typecheck + test:analyze
...
### Task 4.1: Implement recommend.ts 8 rules
...
### Task 4.2: recommend.test.ts — 8 rules × 4 severity coverage
...
### Task 4.3: [VERIFY] Quality checkpoint: typecheck + test:analyze
...
### Task 4.4: report.ts — R1-R7 markdown rendering + Cost Breakdown section
...
### Task 4.5: report.ts — Recommendations section with severity color coding
...
### Task 4.6: index.ts — wire renderCostBreakdown + renderRecommendations + recommendations JSON
...
### Task 4.7: [VERIFY] Quality checkpoint: typecheck + build + test:analyze
...
### Task 4.8: integration.test.ts — Cost Breakdown + Recommendations snapshot
...
### Task 4.9: README — pricing 三步刷新流程章节
...
### Task 4.10: CHANGELOG — single OB-3 entry
...
### Task 4.11: [VE2] E2E smoke — totalCost.usd + recommendations non-null
...
### Task 4.12: [VE3] E2E cleanup — remove temp test artifacts
...
### Task 4.13: [VERIFY-FINAL] Full pipeline + e2e
