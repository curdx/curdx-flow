...
- **`analyze` state schema — `state.files` keyed per-session-path (was single fixture path).** `~/.claude/curdx-flow/state.json` `files` map now keys each entry by the actual `~/.claude/projects/<encoded-cwd>/<session>.jsonl` path resolved at runtime instead of the historical hardcoded `tests/analyze/fixtures/sample.jsonl`. Existing entries keyed by the fixture path are still readable but become stale immediately and are GC'd on next run by the new `cleanupOrphanState` helper (mtime > 30 days OR > 100 keys, whichever fires first). Backwards-compat: legacy keys are not migrated — the GC sweeps them out on first real-transcript run, which is the intended cleanup path for B1 (OB-1).

### Fixed

- **🚨 CRITICAL (B1) — `npx curdx-flow analyze` now reads the user's real `~/.claude/projects/<encoded-cwd>/*.jsonl` transcripts instead of the bundled test fixture (`tests/analyze/fixtures/sample.jsonl`).** Prior versions (v7.1.6 and earlier) shipped a hardcoded `POC_FIXTURE_REL = "tests/analyze/fixtures/sample.jsonl"` constant in `src/analyze/index.ts` that 5 separate code paths (`statSync`, `parseTranscript`, `state.files[fixturePath]`, etc.) keyed off, so the CLI silently emitted *test fixture data* dressed as user analytics for every invocation — an entire major user-facing feature was non-functional. The fix replaces all 5 sites with `resolveTranscriptSource({ cwd, fixtureOverride, sessionFilter })`, which does `realpath(cwd)` (cached), encodes `/` → `-` to match the Claude Code project-dir convention (`~/.claude/projects/-Users-wdx-opc-curdx-flow/`), `readdirSync` 1-level for `*.jsonl`, applies optional `--session <uuid>` filter, and throws `TranscriptNotFoundError` (caught at top-level → friendly 2-line stderr + exit 1) when the project dir is missing. Major user-impact bug — every prior `analyze` output should be considered invalid.

### Added

- **`--session <uuid>` CLI flag on `npx curdx-flow analyze` (OB-1).** New citty arg in `src/flows/analyze.ts` that filters the per-session multi-glob down to a single transcript file (`<uuid>.jsonl`) under `~/.claude/projects/<encoded-cwd>/`. Useful when the user wants to inspect a specific historical session rather than aggregate all sessions in the current cwd. Threaded through `runAnalyze → resolveTranscriptSource({ sessionFilter })` as `Options.session`.
- **`src/analyze/transcript-path.ts` resolver module (OB-1).** New module exporting `resolveTranscriptSource(opts?)`, `TranscriptNotFoundError` class (with `path` + `hint` fields for friendly error display), and `TranscriptSource` union type (`{kind:'fixture'|'real', paths:string[], cwd, realCwd?, encodedDir?}`). Encodes `realpath(cwd)` to match `~/.claude/projects/<encoded>` layout, caches realpath lookups in a module-level Map (D3), and short-circuits to fixture mode when `fixtureOverride` is set (test isolation hook).
- **`cleanupOrphanState(state, currentPaths)` helper in `src/analyze/index.ts` (OB-1, D2).** Two-pass GC over `state.files` map: pass 1 drops entries with `lastModifiedMs > 30 days ago` OR `!existsSync(path)`; pass 2 sorts remaining by `lastModifiedMs` and drops oldest if total > 100 keys. Active session paths (`currentPaths`) are protected from both passes. Wrapped in try/catch inside `finally` block (fail-open per FR-C3) so a thrown GC error never blocks the resume-offset state write.
- **`CURDX_TRANSCRIPT_FIXTURE` env var (test-only escape hatch).** Honored by `resolveTranscriptSource` when set: short-circuits the real-transcript path resolution to a fixture file path and returns `{kind:'fixture', paths:[fixtureOverride]}` without any `realpath` / `readdirSync` work. Used exclusively by `tests/analyze/integration.test.ts` `beforeAll`/`afterAll` to keep the existing snapshot suite green against the bundled `tests/analyze/fixtures/sample.jsonl` without polluting the production code path. Not a user-facing flag — undocumented by design (test isolation only).
- **5 unit tests for `resolveTranscriptSource` + 3 GC tests for `cleanupOrphanState` (OB-1).** `tests/analyze/transcript-path.test.ts` covers (1) cwd `/`→`-` encoding, (2) multi-session glob (3 fixtures → `paths.length === 3`), (3) missing project dir → `TranscriptNotFoundError` with correct `path` + `hint`, (4) `fixtureOverride` short-circuit (`kind === 'fixture'`, no glob), (5) `--session` filter (3 files, only `abc*.jsonl` returned). State GC tests cover (a) 31-day-old entry dropped, (b) 101 entries → oldest 1 dropped, (c) file-gone (`!existsSync`) entry dropped. Active session path is protected across all GC test scenarios. 36/36 analyze tests pass (33 baseline + 3 new GC; resolver tests counted within the 33 baseline post-merge).

#### OB-2 — Decision Event Logging (spec-decision-event-logging)

**Added:**
- **`logHookEvent` function (4-field schema upgrade: `level` / `kind` / `payload` / `correlationId`).** Extends `src/hooks/_shared/error-logger.ts` with a NEVER-throw event-logging API that supersedes the previous `logHookError` single-purpose path. `logHookError` is preserved as a thin redirect (`logHookEvent({ ...ctx, level: 'error', kind: ctx.kind ?? 'unknown' })`) so existing call sites stay byte-equal in serialization. The new schema turns `errors.jsonl` into a unified events log (back-compat name retained); old rows continue to parse via `??` defaults in the parser path so the change is fully additive at the wire level.
- **`EventKind` enum (10 kinds) for structured event taxonomy.** New closed type union (`'matcher_hit' | 'matcher_miss' | 'stop_allow' | 'stop_block' | 'stop_side_effect' | 'task_completion_block' | 'task_completion_pass' | 'context_inject_fail_open' | 'context_inject_success' | 'unknown'`) with a `KNOWN_KINDS` `ReadonlySet` and a `coerceKind(value: unknown): EventKind` coercer that maps anything outside the closed set to `'unknown'` for forward-compat. Replaces the previous "free-string `kind`" pattern (B3) so analyze-side aggregation can group by enum value without string-normalization heuristics.
- **`src/hooks/_shared/correlation.ts` — `buildCorrelationId(stdin, state)` 3-segment helper.** Pure NEVER-throw helper that emits `<session_id>:<task_idx>:<iter>` so cross-hook traces can be join-grepped from a single `errors.jsonl` stream. `session_id` derives from `basename(stdin.transcript_path)` (extension stripped; falls back to `'unknown'` when the SessionStart envelope omits `transcript_path`). `iter` is phase-aware — `state.taskIteration` during execution, `state.globalIteration` otherwise. Every field reads defensively so a partial or null `state`/`stdin` still produces a usable correlation tag (B5: was missing entirely on every prior row).
- **Log rotation: size 10 MB OR age 30 d → `errors.<ISO-ts>-<pid>.jsonl`, retain 5 (D2/D3).** New `shouldRotate(filePath)` dual-gate (first match wins): `statSync(file).size > 10 * 1024 * 1024` OR `(Date.now() - statSync(file).birthtimeMs) > 30 * 24 * 60 * 60 * 1000`. Rotation suffix uses `<YYYYMMDDTHHMMSSZ>-<pid>` collision-free naming. `pruneRotated(dir, keep=5)` globs `errors.*.jsonl`, sorts by mtime desc, unlinks anything past the 5-newest. Throttled: `rotateIfNeeded` only does the `statSync` check every 10th `logHookEvent` call (counter mod 10) so the p99 hot-path budget (~50 ms/spec) stays intact. Replaces the previous "`appendFileSync` forever" path (B4) — without rotation a busy autonomous loop could push `errors.jsonl` past 1 GB inside a few weeks.
- **Cross-platform `safeRename(src, dst)` (4-step fallback chain).** (1) POSIX atomic `renameSync` happy path; (2) Windows `EBUSY` retry chain at 50 ms / 200 ms / 500 ms backoffs (anti-virus / file-handle race); (3) `EXDEV` cross-device fallback uses `copyFileSync` + `unlinkSync`; (4) silent give-up on persistent failure (NEVER-throw contract — the live `errors.jsonl` continues writing even if rotation can't complete this cycle, deferring rename to the next throttle tick). Required because Node's stock `renameSync` throws on `EBUSY` (Windows) and `EXDEV` (`/tmp` ↔ `$HOME` cross-mount), both of which a CI matrix or developer laptop can hit.
- **34 `logHookEvent` sites wired across 4 hooks** — 14 in `stop-watcher.ts` (8 allow / 5 block / 1 side-effect), 9 in `task-completed-verifier.ts` (7 guards / 1 block / 1 success), 8 in `subagent-context-injector.ts` (6 fail-open / 1 success / 1 error), and 3 in `stop-failure-handler.ts` (matcher hit / matcher miss / unknown fallback). Total = 34 sites, 1 buffer above the 33 design-floor — a follow-up CI grep guard in `.github/workflows/ci.yml` is tracked separately (out-of-scope for this spec per design R4).
- **7 new tests in `tests/hooks/event-logger.test.ts`** — case 1 logHookEvent writes all 4 new fields, case 2 `coerceKind` unknown string → `'unknown'`, case 3 rotation triggers at >10 MB on the 10th call (throttle N=10) + live `errors.jsonl` restarts, case 4 rotation triggers at >30 d age, case 5 `buildCorrelationId` 3-segment format from synthetic stdin/state, case 6 round-trip — old single-field row parses with `??` defaults (`level='error'`, `kind='unknown'`, `payload=undefined`, `correlationId=undefined`), case 7 `logHookEvent` NEVER-throws when `appendFileSync` underlying disk is full (mock `fs.appendFileSync` throws → handler still emits `{continue: true}`).
- **`parser.ts` `EventLogRow` schema with `??` defaults for old rows (B3 + B5 read-side).** `loadErrorEntries` in `src/analyze/index.ts` widens the return type to `EventLogRow[]` (superset of `ErrorLogEntry`) and reads the 4 new fields with explicit defaults: `level` falls back to `'error'` if missing or not in the closed `'error'|'info'|'metric'|'decision'` set, `kind` falls back to `'unknown'` if not a string, `payload` is preserved only when it's a non-array object, `correlationId` is preserved only when it's a string. Old single-field rows (`{ts, hook, event, msg, cwd, transcript_path}`) parse cleanly with these defaults so historical `errors.jsonl` lines continue to render in `npx curdx-flow analyze` without migration.

**Fixed (B2-B6):**
- **B2 — `level` no longer hardcoded `'error'`.** Every `logHookEvent` call site supplies an explicit `level` (`'error' | 'info' | 'metric' | 'decision'`); the legacy `logHookError` redirect still pins to `'error'` for back-compat but new sites can emit `'info'` (success cases), `'metric'` (counters / timings), or `'decision'` (allow/block branch points) so analyze-side filtering by severity is now meaningful.
- **B3 — event `kind` is now an enumerated 10-value union (was free-string).** `EventKind` closed type + `coerceKind` coercer means analyze-side `groupBy(kind)` returns a stable bucket count instead of an unbounded long-tail of typos and one-off labels.
- **B4 — log rotation (was `appendFileSync` forever).** Dual-gate (size 10 MB OR age 30 d), retention=5, cross-platform `safeRename` so a busy autonomous loop cannot push `errors.jsonl` past ~50 MB on disk before rotation kicks in (10 MB live × 5 retained).
- **B5 — `correlationId` per row (was missing).** Every `logHookEvent` row now carries a 3-segment `<session_id>:<task_idx>:<iter>` tag so cross-hook traces (e.g. SubagentStart-injection → TaskCompleted-verification → Stop-decision) can be reconstructed by grepping a single `correlationId` value.
- **B6 — `payload` field (was just truncated `msg` string).** Structured `Record<string, unknown>` payloads (e.g. `{ specName, iteration }`) replace the previous "stringify everything into `msg`" pattern, so analyze-side consumers can `JSON.parse(row.payload).specName` directly without re-parsing prose. Redacted via `redactPayload` before `appendFileSync` (existing white-list logic preserved).

**Changed:**
- **`errors.jsonl` is now a UNIFIED events log (back-compat name retained).** The file path is unchanged — `~/.claude/curdx-flow/errors.jsonl` — but the row schema now spans error / info / metric / decision rows instead of error-only rows. Old rows coerce via `??` defaults (`level='error'`, `kind='unknown'`, no `payload`, no `correlationId`) so historical lines continue to parse cleanly. A future spec may rename the file to `events.jsonl` for accuracy; this release prioritizes zero-migration over naming purity.

> **Note on byte-equal preservation (NFR-1):** The 4 existing round-trip tests in `tests/hooks/error-logger.test.ts` (legacy `logHookError` shape) pass unmodified because `logHookError` is now a thin `logHookEvent({ ...ctx, level: 'error', kind: ctx.kind ?? 'unknown' })` redirect — the wire-level JSON for legacy callers is byte-identical to v7.1.6 output. 110/110 hook tests + 36/36 analyze tests + drift gates all pass on the v7.1.7 build.
